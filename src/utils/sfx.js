...
function detectStepOnsets(buffer) {
  ...
  const threshold = Math.max(
    median * 1.9,
    sorted[Math.floor(sorted.length * 0.97)] * 0.45,
  );

  const minGapHops = Math.ceil(
    (STEP_MIN_GAP_SECONDS * buffer.sampleRate) / hop,
  );
  const onsets = [];
  let last = -minGapHops;
      ...
      .then((buffer) => {
        ...
        if (name === "steps") {
          ...
          if (import.meta.env.DEV)
            console.info(`[sfx] steps: ${onsets.length} onsets detected`);
        }
      })
...
export function playSfx(name, { gain = 1, rate = 1, rateJitter = 0.05 } = {}) {
  shoot(buffers[name], {
    gain: gain * jitter(0.12),
    rate: rate * jitter(rateJitter),
  });
  if (import.meta.env.DEV && window.__sfxLog && name in window.__sfxLog)
    window.__sfxLog[name]++;
}

// One-shot you can cut short — used for the absorb "charge" so the blast can
// cleanly take over on release. Returns a handle; stop() fades out fast (no click).
export function playStoppable(
  name,
  { gain = 1, rate = 1, rateJitter = 0.05 } = {},
) {
...
// norm: 0 (stopped) → 1 (top speed). Pitch + volume rise with it, smoothed on
// the audio thread. Cheap enough to call every frame.
...
// Vite HMR: close the AudioContext + stop loops/engine on hot-replace, so editing
// doesn't accumulate orphaned, still-running AudioContexts (browsers cap them
// and each keeps an audio render thread alive). Prod builds strip this block.
if (import.meta.hot) {
...
export function playStep({ gain = 0.55 } = {}) {
  ...
  shoot(entry, {
    gain: gain * jitter(0.18),
    rate: jitter(0.07),
    duration: STEP_SLICE_SECONDS,
  });
